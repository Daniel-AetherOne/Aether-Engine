...
def _get_vertical_or_404(vertical: str):
    ...
    try:
        ...
        # KeyError message already includes "Available: [...]"
...
def intake_by_vertical(request: Request, vertical: str):
    v = _get_vertical_or_404(vertical)
    return v.render_intake_form(request, lead_id=str(uuid.uuid4()))


@router.post("/{vertical}/lead")
async def create_lead_by_vertical(
    request: Request, vertical: str, db: Session = Depends(get_db)
):
    v = _get_vertical_or_404(vertical)
...
def intake_painters_us(request: Request):
    v = get_vertical(DEFAULT_VERTICAL)
    return v.render_intake_form(request, lead_id=str(uuid.uuid4()))


@router.post("/lead")
async def create_lead_legacy(request: Request, db: Session = Depends(get_db)):
    # legacy: default vertical
    return await create_lead_by_vertical(request, DEFAULT_VERTICAL, db)
