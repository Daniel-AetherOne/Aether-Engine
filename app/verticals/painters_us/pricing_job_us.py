...
def price_job_from_vision(vision_output: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for idx, s in enumerate(surfaces):
        ...
        if r.get("status") == "needs_review" or r.get("needs_review") is True:
            ...
            low = float(est.get("low_usd") or 0.0)
            high = float(est.get("high_usd") or 0.0)
            range_low += low
            range_high += high
            ...
            per_item.update(
                {
                    ...
                    "materials_usd": None,
                    "total_usd": None,
                    "estimate_range": {"low_usd": low, "high_usd": high},
                }
            )
        elif r.get("status") == "priced_with_margin":
            # Totals for job
            labor = float(r.get("labor_usd") or 0.0)
            materials = float(r.get("materials_usd") or 0.0)
            ...
            # Try to capture unit_price_usd from line_items[0] if present
            line_items = r.get("line_items") or []
            unit_price = None
            if line_items and isinstance(line_items, list):
                unit_price = line_items[0].get("unit_price_usd")
            ...
            per_item.update(
                {
                    ...
                    "materials_usd": None,
                    "total_usd": None,
                    "estimate_range": {"low_usd": 0.0, "high_usd": 0.0},
    ...
    if any_needs_review:
        return {
            ...
            "total_usd": None,
            "estimate_range": {
                "low_usd": round(range_low, 2),
                "high_usd": round(range_high, 2),
            },
            "items": items,
        }
    ...
    return {
        ...
        "pricing_ready": True,
        "needs_review": False,
        "labor_usd": round(labor_total, 2),
        "materials_usd": round(materials_total, 2),
        "total_usd": round(total_total, 2),
        "items": items,
    }
